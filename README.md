...
## 4. 역할
- **데이터베이스 설계 및 총괄** - PK, FK, Sequence, Trigger 등등 전반적인 담당
- **카카오 로그인/회원가입 API, Daum 주소검색 API 구현**
- **회원가입 페이지 구현** - 기입란에 맞지 않는 입력 감지필터, 아이디/닉네임 중복확인(Ajax)
- **상품 목록 및 카테고리 구현** - 상품목록 페이징 및 카테고리 선택 시 localStorage를 이용하여 카테고리 글씨 색상 변경 유지
- **장바구니 페이지 구현** - 전체선택/개별선택, 실시간 구매금액 출력
- **결제페이지 페이지 배송지 선택 구현** - 회원가입 시 입력한 주소지 / Daum 주소검색 API로 배송지 입력
