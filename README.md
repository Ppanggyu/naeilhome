...
### Trigger 작동 시점
회원 가입 시 포인트 추가
- 회원가입 시 기본 1000포인트 부여

주문 트리거 ( 재고 감소, 잔여 포인트 감소 )
- 상품구매 시 주문상품의 주문상태가 '결제완료'로 들어감
- 결제완료 + INSERT문을 트리거가 인식 시 상품재고 감소 및 주문상품의 사용포인트 기반으로 포인트 실시간 업데이트
- 주문상품의 주문상태가 '구매확정'으로 변경될 때 트리거 실행
- 구매한 상품의 금액 기준 1%의 적립

교환 취소 시 재고 증가 및 포인트 반환, 반품/교환 시 ordersJoin의 orderStatement 적용
- 취소/반품/교환 테이블에 데이터 추가 시 처리상태가 주문상품의 주문상태가 업데이트
- 취소/반품/교환 테이블에 처리상태가 '취소'일 시 상품재고 복구 + 사용포인트 반환
- 취소/반품/교환 테이블에 처리상태가 '반품완료'일 시 사용포인트 반환

리뷰 작성 시 150포인트 적립
- 리뷰 테이블에 데이터 추가 시 150 포인트 적립
- 추가로 포인트 테이블 값이 실시간으로 변동
